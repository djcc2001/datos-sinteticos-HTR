...
def ctc_greedy_decoder(probs_seq, blank_index=None):
    ...
    # remove blank indexes (repo convention: blank is last class)
    if blank_index is None:
        blank_index = int(np.array(probs_seq).shape[1] - 1)
...
def make_one_hot(labels, len_labels, n_class):
    ...
    for i in range(len(labels)):
        L = int(len_labels[i].item() if hasattr(len_labels[i], 'item') else len_labels[i])
        # labels son índices 0..n_class-1 (0-indexed)
        idxs = labels[i, :L].long().clamp(0, n_class - 1).cpu().numpy()
        one_hot[i, np.arange(L), idxs] = 1
    return one_hot
...
def rand_clip(imgs, img_lens, min_clip_width=64):
    ...
    # Images are normalized to [-1, 1] with white background ~= +1.
    pad_imgs = np.ones((imgs.shape[0], 1, imgs.shape[2], max_img_len), dtype=np.float32)
