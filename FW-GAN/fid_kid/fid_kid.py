...
def get_activations(data_source, n_batches, max_img_width, model, dims, device):
    ...
    for batch in tqdm(data_source, total=n_batches):
        ...
        # Kiểm tra và thay vùng toàn -1 từ img_lens trở đi thành 1
        batch_size, _, height, width = imgs.size()
        for i in range(batch_size):
            len_i = int(img_lens[i].item())  # Độ dài thực tế của ảnh i
            if len_i < width:  # Nếu ảnh ngắn hơn chiều rộng hiện tại
                padding_region = imgs[i, :, :, len_i:]  # Vùng từ len_i trở đi
                # Kiểm tra xem vùng này có toàn -1 không
                if torch.all(padding_region == -1):
                    imgs[i, :, :, len_i:] = 1.0  # Thay toàn bộ vùng bằng 1
        ...
        # Kiểm tra và lặp lại kênh nếu là ảnh grayscale
        if imgs.size(1) == 1:
            imgs = imgs.repeat(1, 3, 1, 1)

        # Lấy chiều cao hiện tại và tính chiều rộng mục tiêu
        height, width = imgs.size(-2), imgs.size(-1)
        target_width = 4 * height  # Tỷ lệ 1:4 => width = 4 * height

        # Điều chỉnh chiều rộng
        if width != target_width:
            if width < target_width:
                # Pad nếu width nhỏ hơn target_width
                pad_width = target_width - width
                imgs = torch.nn.functional.pad(imgs, [0, pad_width, 0, 0], value=1.0)
            elif width > target_width:
                imgs = imgs[:, :, :, :target_width]
        ...
        # Nếu output không phải scalar, áp dụng global spatial average pooling
