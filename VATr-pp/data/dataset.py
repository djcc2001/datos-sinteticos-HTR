...
class TextDataset:
    ...
    def __init__(
        ...
        max_width: int = 192,
        max_label_len: int | None = 32,
        allowed_chars: str | None = None,
        restrict_author_ids: list[int] | None = None,
    ):
        self.NUM_EXAMPLES = num_examples
        self.debug = debug
        self.min_virtual_size = min_virtual_size
        self.max_width = int(max_width)
        self.max_label_len = max_label_len if max_label_len is None else int(max_label_len)
        self.allowed_chars = allowed_chars
        self.restrict_author_ids = set(restrict_author_ids) if restrict_author_ids else None
        ...
        if not isinstance(raw, dict):
            raise ValueError(f"Unexpected dataset format in {base_path}")
        ...
        for subset_name in subset_aliases[split]:
            subset_data = raw.get(subset_name)
            if isinstance(subset_data, dict):
                explicit_subset = subset_data
                break

        if explicit_subset is not None:
            self.IMG_DATA = {
                author_id: [self._normalize_sample(sample) for sample in samples]
                for author_id, samples in explicit_subset.items()
            }
        else:
            # Backward compatibility for legacy pickles without explicit split keys.
            merged = {}
            if any(k in raw for k in ("train", "test", "val", "validation")):
                for subset_data in raw.values():
                    if not isinstance(subset_data, dict):
                        continue
                    for author_id, samples in subset_data.items():
                        merged.setdefault(author_id, []).extend(self._normalize_sample(sample) for sample in samples)
            else:
                merged = {
                    author_id: [self._normalize_sample(sample) for sample in samples]
                    for author_id, samples in raw.items()
                }
            self.IMG_DATA = merged

        if 'None' in self.IMG_DATA.keys():
            del self.IMG_DATA['None']

        def _to_int_author(a):
            if isinstance(a, int):
                return a
        ...
        def _strict_target_split(author_int: int) -> str | None:
            # STRICT split mapping requested by user:
            # IAM:
            #   0–1399 -> TRAIN
            #   1400–1469 -> VALIDATION
            #   1470–1538 -> TEST
            # Spanish:
            #   1539,1540,1542,1543 -> TRAIN
            #   1541 -> VALIDATION
            #   1544 -> TEST
            if 0 <= author_int <= 1399:
                return "train"
            if 1400 <= author_int <= 1469:
                return "val"
            if 1470 <= author_int <= 1538:
                return "test"
            if author_int in {1539, 1540, 1542, 1543}:
                return "train"
            if author_int == 1541:
                return "val"
            if author_int == 1544:
        ...
        if self.restrict_author_ids is not None:
            filtered = {}
            for author_id, samples in self.IMG_DATA.items():
                author_int = _to_int_author(author_id)
                if author_int is None or author_int not in self.restrict_author_ids:
                    continue
                filtered[author_id] = samples
            self.IMG_DATA = filtered

        # Apply strict split only for legacy pickles without explicit train/val/test subsets.
        if strict_split and explicit_subset is None:
            selected = {}
            matched_any = False
            for author_id, samples in self.IMG_DATA.items():
                author_int = _to_int_author(author_id)
                if author_int is None:
                    continue
                target = _strict_target_split(author_int)
                if target is None:
                    continue
                matched_any = True
                if target == split:
                    selected[author_id] = samples

            if matched_any:
                self.IMG_DATA = selected

        self.alphabet = ''.join(sorted(set(''.join(d['label'] for d in sum(self.IMG_DATA.values(), [])))))
        # Stable ordering matters for writer-class indices.
        self.author_id = sorted(
            list(self.IMG_DATA.keys()),
            key=lambda a: (_to_int_author(a) is None, _to_int_author(a) if _to_int_author(a) is not None else str(a)),
        )
    ...
    @classmethod
    def _normalize_sample(cls, sample: dict) -> dict:
        sample = dict(sample)
        if 'label' in sample:
            sample['label'] = cls.normalize_label(sample['label'])
        return sample
    ...
    def __getitem__(self, index):
        ...
        label_str = self.normalize_label(word_data['label'])
        if self.allowed_chars is not None:
            label_str = ''.join(c if c in self.allowed_chars else ' ' for c in label_str)
        if self.max_label_len is not None:
            label_str = label_str[: self.max_label_len]
        real_labels = label_str.encode('utf-8')

        imgs = [np.array(self.IMG_DATA_AUTHOR[idx]['img'].convert('L')) for idx in random_idxs]
        slabels = []
        for idx in random_idxs:
            s = self.normalize_label(self.IMG_DATA_AUTHOR[idx]['label'])
            if self.allowed_chars is not None:
                s = ''.join(c if c in self.allowed_chars else ' ' for c in s)
            if self.max_label_len is not None:
                s = s[: self.max_label_len]
            slabels.append(s.encode('utf-8'))

        max_width = self.max_width  # cap for memory
        ...
        for img in imgs:
            ...
            clipped = min(img_width, max_width)
            output_img[:, :clipped] = img[:, :clipped]

            imgs_pad.append(self.transform(Image.fromarray(output_img.astype(np.uint8))))
            imgs_wids.append(clipped)
...
class TextCollator(object):
    ...
    def __call__(self, batch):
        ...
        wcls = torch.tensor([item['wcl'] for item in batch], dtype=torch.long)
        swids = torch.tensor([item['swids'] for item in batch], dtype=torch.int32)
