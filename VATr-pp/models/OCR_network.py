...
class strLabelConverter(object):
    ...
    def __init__(self, alphabet, ignore_case=False, unknown_char: str = " "):
        ...
        self.unknown_char = unknown_char if unknown_char in self.alphabet else None
    ...
    def encode(self, text):
        ...
        result = []
        results = []
        unknown_index = self.dict.get(self.unknown_char, 0) if self.unknown_char is not None else 0
        for item in text:
            if isinstance(item, bytes):
                item = item.decode('utf-8', 'strict')
            length.append(len(item))
            for char in item:
                index = self.dict.get(char, unknown_index)
                result.append(index)
