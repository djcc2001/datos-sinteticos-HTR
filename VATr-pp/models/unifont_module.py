...
class UnifontModule(torch.nn.Module):
    ...
    def get_symbols(self, input_type):
        ...
        any_mat = next(iter(all_symbols.values()))
        fallback = np.zeros_like(any_mat, dtype=np.float32).flatten()
        symbols = []
        missing_chars = []
        for char in self.alphabet:
            mat = all_symbols.get(ord(char), None)
            if mat is None:
                missing_chars.append((char, ord(char)))
            im = fallback if mat is None else mat.flatten()
            symbols.append(im)

        if missing_chars:
            preview = ", ".join([f"{char}(U+{code:04X})" for char, code in missing_chars[:12]])
            print(
                f"WARNING: missing {len(missing_chars)} glyphs in files/{input_type}.pickle; "
                f"falling back to blank glyphs. Sample: {preview}"
            )
