...
class OCRAugment(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for augmentation in augmentations:
            # ElasticTransform can fail on very narrow words due to internal reflect padding.
            if isinstance(augmentation, torchvision.transforms.ElasticTransform):
                if x.size(-2) <= 28 or x.size(-1) <= 28:
                    continue

            try:
                x = augmentation(x)
            except RuntimeError as e:
                # Skip only augmentation-related shape/padding issues and continue training.
                if "Padding size should be less than the corresponding input dimension" in str(e):
                    continue
                raise
