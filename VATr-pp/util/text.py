...
class FileTextGenerator(TextGenerator):
    def __init__(self, max_length: int, file_path: str, alphabet: list):
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            self.words = [unicodedata.normalize('NFC', line) for line in f.read().splitlines()]
    ...
    def __init__(self, max_length: int, file_path: str, alphabet: list):
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                annotation = unicodedata.normalize('NFC', ",".join(annotation))
...
def get_generator(args):
    full_alphabet = ''.join(dict.fromkeys(args.alphabet + args.special_alphabet))
    if args.corpus == "standard":
        if args.words_path.endswith(".csv"):
            generator = CVLFileTextIterator(20, args.words_path, full_alphabet)
        else:
            generator = FileTextGenerator(20, args.words_path, full_alphabet)
    else:
        generator = IAMTextGenerator(20, "files/IAM-32.pickle", 'train')

    if args.text_augment_strength > 0:
        if args.text_aug_type == 'proportional':
            return ProportionalAugmentedGenerator(20, generator, full_alphabet, args.text_augment_strength)
...
if __name__ == "__main__":
    raise SystemExit("This module is not intended to be executed as a script.")
