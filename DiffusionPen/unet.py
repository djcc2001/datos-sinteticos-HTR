...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, length, *args):
        ...
        # Preserve autocast settings so recompute in backward matches forward.
        try:
            first = ctx.input_tensors[0]
            ctx._autocast_device_type = first.device.type
        except Exception:
            ctx._autocast_device_type = "cuda"
        ctx._autocast_enabled = torch.is_autocast_enabled()
        # GPU dtype helper exists even if device is CPU; harmless.
        ctx._autocast_gpu_dtype = torch.get_autocast_gpu_dtype()
    ...
    def backward(ctx, *output_grads):
        # Keep original dtype to play nicely with AMP/autocast.
        ...
        with torch.enable_grad():
            ...
            # Recompute under the same autocast mode as forward.
            with torch.autocast(
                device_type=getattr(ctx, "_autocast_device_type", "cuda"),
                dtype=getattr(ctx, "_autocast_gpu_dtype", torch.float16),
                enabled=getattr(ctx, "_autocast_enabled", False),
            ):
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        # Keep the (frozen) text encoder as an *unregistered* attribute to avoid:
        # - duplicating it in EMA copies (huge memory hit on Colab/T4)
        # - saving it inside UNet checkpoints
        # Training code should precompute text embeddings and pass a tensor as `context`.
    ...
    def forward(self, x, timesteps=None, context=None, y=None, mix_rate=None, style_extractor=None, **kwargs):
        ...
        if self.interpolation:
            ...
            if style_extractor is not None:
                if y.dim() == 2:
                    ...
                    raise ValueError(f"Unexpected style tensor shape: {tuple(y.shape)}")
        ...
        if context is not None:
            ...
            # - a dict/BatchEncoding compatible with HF text encoders, or
            # - a precomputed tensor of hidden states [B, L, D].
            if isinstance(context, Mapping):
                if self.text_encoder is None:
                    raise ValueError("UNetModel received tokenized `context` dict but no text_encoder was provided.")
                context = self.text_encoder(**context).last_hidden_state
            elif not torch.is_tensor(context):
                raise TypeError("context must be a dict/BatchEncoding or a torch.Tensor")

            if self.cont_dim == 320 and context.shape[-1] != 320:
